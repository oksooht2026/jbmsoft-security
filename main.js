...
const remoteUpdater = require('./security/remote-updater');
    ...
    { type: 'separator' },
    {
      ...
      click: () => {
        if (isEnginePaused) {
          ...
          osEngine.scanExistingUsbDrives(onUsbFileEvent).catch(() => {});
...
async function runServerSync({ needsApprovals = false } = {}) {
  const result = await serverSync.syncWithServer({
    ...
    updateStatus: remoteUpdater.getReportableStatus()
  });
  ...
  if (result.update) {
    // 관리자 패널에서 배포한 원격 업데이트 — 대상·최신 버전인 경우에만 무인 자동 설치
    remoteUpdater.checkAndApply(result.update).catch(err => {
      console.error('[Main] 원격 업데이트 처리 실패:', err.message);
    });
  }
...
function setupApprovalWorkflow() {
  ...
  osEngine.setUsbGrantChecker((drive) => approvalManager.isGranted('usb_connect', drive));
  osEngine.setUsbApprovalRequestCallback((drive, fs) => {
    // 자동 승인 요청 제거 — 사용자가 팝업에서 사유 입력 후 직접 전송
    appendSecurityLog({ type: 'warning', message: `[USB 승인 대기] ${drive}: 드라이브 — 사유 입력 후 승인 요청 필요` });

    // UI 팝업: 사유 입력 인터페이스 포함하여 표시
    if (mainWindow) {
      if (mainWindow.isMinimized()) mainWindow.restore();
      mainWindow.show();
      mainWindow.focus();
      if (isPageReady) {
        mainWindow.webContents.send('usb-blocked', { drive, fs });
      }
    }

    // Windows 네이티브 알림창
    const targetWin = mainWindow && mainWindow.isVisible() ? mainWindow : null;
    dialog.showMessageBox(targetWin, {
      type: 'warning',
      title: '옥수하이테크 보안솔루션',
      message: '이동식 디스크(USB) 연결 차단',
      detail: `보안 정책에 의해 [${drive}:] 드라이브의 연결이 차단되었습니다.\n\n앱에서 사용 사유를 입력하면 관리자에게 승인 요청이 전송됩니다.`,
      buttons: ['확인'],
      defaultId: 0,
      noLink: true
    }).catch(() => {});
...
async function onUsbFileEvent(event, filePath) {
  const sec = store.get('security', {});
  if (!sec.usbGuard) return;

  if (event !== 'add') {
    const key = `${event}:${filePath.toLowerCase()}`;
    const now = Date.now();
    if (usbEventCooldown.get(key) && now - usbEventCooldown.get(key) < USB_EVENT_COOLDOWN_MS) return;
    usbEventCooldown.set(key, now);
    serverSync.sendLog('usb_file_event', 'info',
      `USB 파일 ${event === 'unlink' ? '이동/삭제' : '변경'}: ${path.basename(filePath)}`,
      { filePath, event }
    ).catch(() => {});
    return;
  }

  const grantId = filePath;
  if (approvalManager.isGranted('usb_file_transfer', grantId)) return;

  const key = `add:${filePath.toLowerCase()}`;
  const now = Date.now();
  if (usbEventCooldown.get(key) && now - usbEventCooldown.get(key) < USB_EVENT_COOLDOWN_MS) return;
  usbEventCooldown.set(key, now);
  ...
  try {
    if (fs.existsSync(filePath)) quarantined = quarantineFile(filePath);
  } catch (e) {
    console.warn('[Main] USB 파일 격리 실패:', e.message);
  }

  const filename = path.basename(filePath);
  const ext = getFileExt(filePath);
  const drive = (filePath.match(/^([A-Za-z]):/) || [])[1]?.toUpperCase() || '';

  await approvalManager.createRequest('usb_file_transfer', {
    filename,
    filePath,
    ...
    drive,
    destination: 'USB 복사',
    reason: `USB 파일 이동 승인 요청 — ${filename}`
  });

  appendSecurityLog({ type: 'warning', message: `[USB 승인 대기] ${filename} — 관리자 승인 후 USB 복사 가능` });
  serverSync.sendLog('usb_file_pending', 'warning',
    `[승인 대기] USB 파일: ${filename}`,
    { filePath, quarantinedTo: quarantined, drive }
  ).catch(() => {});
  ...
  dialog.showMessageBox({
    type: 'warning',
    title: '옥수하이테크 보안솔루션',
    message: 'USB 파일 복사 차단 및 격리',
    detail: `보안 정책에 의해 USB로의 파일 복사/이동이 차단되었습니다.\n\n파일명: ${filename}\n\n관리자에게 승인 요청이 자동으로 전송되었습니다. 승인 완료 시 파일이 정상 복원 및 저장됩니다.`,
    buttons: ['확인'],
    defaultId: 0,
    noLink: true
  }).catch(() => {});

  console.log(`[Main] USB 파일 승인 요청: ${filePath}`);
}
...
function startSecurityEngineDeferred() {
  setTimeout(() => {
    ...
    console.log('[Main] 보안 엔진 기동 (UI 안정화 후)');
    osEngine.startEngine(onUsbFileEvent);
    startFileWatcherFromStore();
    osEngine.scanExistingUsbDrives(onUsbFileEvent);
...
ipcMain.handle('set-store', (event, key, value) => {
  ...
  if (key === 'security' && value) {
    osEngine.updateEnginePolicy({
      usb_blocking_enabled: value.usbGuard,
      blocked_extensions: value.blockedExtensions,
      clipboard_monitoring_enabled: value.clipboardGuard
    });
    extensionGuard.updatePolicy({ blocked_extensions: value.blockedExtensions, usb_blocking_enabled: value.usbGuard });
...
// USB 사유 입력 후 승인 요청 전송
ipcMain.handle('usb-submit-approval', async (event, { drive, fs, reason }) => {
  try {
    const req = await approvalManager.createRequest('usb_connect', { drive, fs, reason });
    appendSecurityLog({ type: 'warning', message: `[USB 승인 요청] ${drive}: 드라이브 — 사유: ${reason || '(없음)'}` });
    if (mainWindow && isPageReady) {
      mainWindow.webContents.send('usb-approval-sent', { drive, reason });
    }
    return { ok: true, id: req && req.id };
  } catch (err) {
    console.error('[Main] usb-submit-approval 오류:', err.message);
...
ipcMain.handle('resume-engine', async () => {
  osEngine.startEngine(onUsbFileEvent);
  startFileWatcherFromStore();
  await osEngine.scanExistingUsbDrives(onUsbFileEvent);
  isEnginePaused = false;
  updateTrayMenu();
  if (tray) tray.setToolTip('옥수하이테크 보안솔루션 - 보안 활성화 중');
...
app.whenReady().then(async () => {
  ...
  if (process.platform === 'win32') {
    app.setAppUserModelId('com.oksoohitech.security');
  }
  ...
  // 원격 업데이트 모듈 초기화 (현재 빌드 버전 서버에 보고 + 무인 자동 설치 준비)
  serverSync.setAppVersion(app.getVersion());
  remoteUpdater.init({
    app,
    serverSync,
    store,
